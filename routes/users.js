...
router.post('/registerdoctor',(req,res)=>{
  console.log(req.body.email);
    ...
    Doctor.findOne({ username:newDoctor.username}).exec((err, user) => {
      if(user)
      {
        ...
        Doctor.findOne({email:newDoctor.email}).exec((err, user) => {
          if(user)
          {
            //console.log("same name");
            res.json({success:false,msg:'Same username or email exists already'});
...
router.post('/registerchemist',(req,res)=>{
    ...
    Chemist.findOne({ username:newChemist.username}).exec((err, user) => {
      if(user)
      {
        ...
        Chemist.findOne({email:newChemist.email}).exec((err, user) => {
          if(user)
          {
            //console.log("same name");
            res.json({success:false,msg:'Same username or email exists already'});
...
router.post('/profilechem',(req,res,next)=>{
    console.log(req.body.chemuname);
  Chemist.findOne({username:req.body.chemuname},{'_id':0,'password':0},(err,chemist)=>{
   console.log(chemist);
   res.json(chemist); 
  });
