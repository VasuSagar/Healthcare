...
export class CheckoutprescriptionComponent implements OnInit {
  ...
  chemuname:string; //chemistusername
  shopname:string;

  constructor(private transferService:TransferService,private router:Router,
    private authService:AuthService,
    private flashMessage:FlashMessagesService,private globals: Globals) { }

  ngOnInit(): void {
    console.log(this.globals.uname);
    var obj={chemuname:this.globals.uname};

    this.authService.getProfileChemist(obj).subscribe(chemist=>

      {
        this.shopname=chemist.shop;
      });
